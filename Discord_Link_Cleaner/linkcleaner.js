...
const helpMessage = new Discord.MessageEmbed()
	...
	.setThumbnail(image)
	.addFields(
				{ name: 'help', value: 'For help type *'+prefix+'help*' },
				{ name: 'clean links', value: 'to clean links type\n*'+prefix+'clean https://google.com*' },
				{ name: 'say Hi', value: 'to say hi type *'+prefix+'hello*' },
			)
    ...
    if (command === 'ping') {
	    ...
	    try {
	        if (!cleanLink(args, message)) {
	            if (cleanLink(args[1].substring(0, args[1].length - 23), message)) {
	            }
                    else {
